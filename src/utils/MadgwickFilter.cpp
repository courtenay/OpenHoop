...
void MadgwickFilter::setBeta(float b) {
    beta = b;
}

void MadgwickFilter::setSampleFreq(float freq) {
    sampleFreq = freq;
}
...
float MadgwickFilter::getPitch() const {
    ...
    if (fabsf(sinp) >= 1.0f) {
        return copysignf(90.0f, sinp);  // Use 90 degrees if out of range
    }
...
void MadgwickFilter::getGravityVector(float& gx, float& gy, float& gz) const {
    // Rotate [0, 0, 1] (world down) by inverse quaternion to get gravity in body frame
    gx = 2.0f * (q1 * q3 - q0 * q2);
    gy = 2.0f * (q0 * q1 + q2 * q3);
    gz = q0 * q0 - q1 * q1 - q2 * q2 + q3 * q3;
}

float MadgwickFilter::getDownAngle() const {
    // Get gravity vector in body frame
    float gx, gy, gz;
    getGravityVector(gx, gy, gz);

    // Calculate angle in XZ plane (around Y axis, which is through the hoop)
    // This gives us where "down" points relative to the hoop's LED positions
    float angle = atan2f(gx, gz) * 180.0f / M_PI;
